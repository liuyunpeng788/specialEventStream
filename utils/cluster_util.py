...
from algorithm.util_seg import get_stop_words
...
def getClusterResult(articles):
    '''
    对当前的文章进行聚类
    :param articles: 文章集合，list[Article]
    :return:  聚类的文章列表，首篇文章表示的是聚类信息   List[[Article]]
    '''
...
def ArtilceToArticleClusterDetail(specialId,topicId,articles):
    ...
    :param topicId: 聚类id
    :param articles: 文章列表
    :return: 聚类详情列表 List[EsSpecialInfos]
    '''
...
def parseFirstArtilceToCluster(clusterIndex,artilce,article_num = 0,negatice_num = 0):
    '''
    将聚类的首篇文章转换为聚类对象（EsSpecialAgg），用于存储入库。
    :param clusterIndex: 聚类id
    :param artilce: 文章
    :param refedTimes : 被文章数
    :return: EsSpecialAgg 待入库的聚类对象
    '''
...
def getMostSimilarCluster(historyClusterInfo,toClusterList):
    ...
    :return:  相似文章字典。 key: 新增文章聚类的序号，value : 历史聚类序号
    '''
...
###### zyy added ######
def getSimilarCluster(historyClusterInfo,toClusterList):
    '''
    获取待合并的聚类与历史聚类信息的合并映射关系  如{1:3,2:None} 表示待合并的聚类1 与历史聚类3 相似,
    待合并的聚类2没有相似的历史聚类信息，属于新增聚类
    :param historyClusterInfo: 历史聚类信息
    ...
    historyClusterArticles = []  ## 历史聚类信息
    toClusterArticles = []  ## 待聚类信息
    historyClusterPhrase = []  ## 历史聚类信息
    toClusterPhrase = []  ## 待聚类信息

    for historyCluster in historyClusterInfo:
        historyClusterPhrase.append(historyCluster.phrase)
        historyClusterArticles.append(historyCluster.topicDesc)

    for articles in toClusterList:
        toClusterPhrase.append(articles[0].phrase)
        toClusterArticles.append(articles[0].title)

    simDic = {}
    keywords = get_stop_words()

    for i in range(len(toClusterPhrase)):
        phrase = toClusterPhrase[i].split(',')
        title = toClusterArticles[i]
        segments = [item.word for item in HanLP.segment(title) if item.word not in keywords]
        simtitle = Simhash(' '.join(segments))
        for id in range(len(historyClusterPhrase)):
            history_phrase = historyClusterPhrase[id].split(',')
            history_title = historyClusterArticles[i]
            his_simtitle = Simhash(' '.join(util_seg_before_rebuild.segment(history_title)))
            if len([x for x in phrase if x in history_phrase]) >= 2 or simtitle.distance(his_simtitle) < 15:
                simDic[i] = id
                break
        if i not in simDic:
            simDic[i] = None
...
def mergeClusterInfo(historyClusterInfo,clusterList,lastClusterIndex):
    ...
    try:
        if historyClusterInfo is None or len(historyClusterInfo) == 0:
            ...
            ######## zyy modified #########
            clusterMergeDic = getSimilarCluster(historyClusterInfo, clusterList)
            ######################

            print(clusterMergeDic)
